...
namespace fluentiter {
  ...
  public:
    ...
    T min() {
      return reduce([](auto acc, auto elem) { return std::min(acc, elem); },
                    std::numeric_limits<T>::max());
    }

    T max() {
      return reduce([](auto acc, auto elem) { return std::max(acc, elem); },
    ...
    size_t count() {
      return reduce([](auto acc, auto) { return acc + 1; }, 0);
    }
